...
static const char *const omen_thermal_profile_boards[] = {
    ...
    "8917", "8918", "8949", "894A", "89EB", "8A15", "8A42", "8BA9", "8BAD", "8E41",
};
...
static int platform_profile_omen_get_ec(enum platform_profile_option *profile) {
  ...
  pr_info("platform_profile_omen_get_ec: tp = 0x%x\n", tp);
  if (tp < 0)
    return tp;
...
static int platform_profile_omen_set_ec(enum platform_profile_option profile) {
  ...
  const char *board_name = dmi_get_system_info(DMI_BOARD_NAME);
  bool is_8ba9 = board_name && strcmp(board_name, "8BA9") == 0;
...
static bool is_victus_s_fan(void) {
  const char *board_name = dmi_get_system_info(DMI_BOARD_NAME);
  bool is_8ba9 = board_name && strcmp(board_name, "8BA9") == 0;
  return is_victus_s_board || is_8ba9;
}
...
static int thermal_profile_setup(struct platform_device *device) {
  ...
  pr_info("thermal_profile_setup: is_omen = %d, is_victus = %d, is_victus_s = %d\n",
          is_omen_thermal_profile(), is_victus_thermal_profile(), is_victus_s_thermal_profile());

  if (is_omen_thermal_profile()) {
    err = platform_profile_omen_get_ec(&active_platform_profile);
    pr_info("thermal_profile_setup: platform_profile_omen_get_ec returned %d\n", err);
    if (err < 0)
      return err;
...
static umode_t hp_wmi_hwmon_is_visible(const void *data,
  ...
  switch (type) {
  case hwmon_pwm:
    if (attr == hwmon_pwm_input && !is_victus_s_fan())
      return 0;
    return 0644;
  case hwmon_fan:
    if (is_victus_s_fan()) {
